...
def test_program_runs_instruction_for_second_time():
    ...
    acc, instr = run_code(instructions_to_read)
    assert acc == 10
    assert instr == "acc +1"
...
def test_sample_input():
    ...
    acc, instr = run_code(puzzle_input)
    assert acc == 5
    assert instr == "acc +1"
